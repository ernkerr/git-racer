...
// ── Preset "three" with green-themed colors for Git Racer ────────────────────
const GIT_RACER_PRESET = {
  onSpeedUp: () => {},
  onSlowDown: () => {},
  ...
  movingAwaySpeed: [10, 30] as [number, number],
  movingCloserSpeed: [-80, -130] as [number, number],
  ...
  colors: {
    ...
    // Green theme to match Git Racer
    leftCars: [0x005c26, 0x00c853, 0x00ff87],
    rightCars: [0xf0f0f0, 0xcccccc, 0x999999],
    sticks: 0x00c853,
  },
};

type EffectOptions = typeof GIT_RACER_PRESET;
...
class CarLights {
  webgl: HyperspeedApp;
  options: EffectOptions & { distortion: Distortion };
  colors: number[];
  speed: [number, number];
  fade: THREE.Vector2;
  mesh!: THREE.Mesh;
...
class LightsSticks {
  ...
  constructor(webgl: HyperspeedApp, options: EffectOptions & { distortion: Distortion }) {
    this.webgl = webgl;
    this.options = options;
  }
  ...
  init() {
    ...
    const sticksRaw = options.colors.sticks as any;
    let colors: THREE.Color[];
    if (Array.isArray(sticksRaw)) {
      colors = sticksRaw.map((c: number) => new THREE.Color(c));
    } else {
      colors = [new THREE.Color(sticksRaw)];
    }
...
class HyperspeedApp {
  ...
  constructor(container: HTMLElement, options: EffectOptions & { distortion: Distortion }) {
    this.options = options;
  ...
  setSize(width: number, height: number, _updateStyles: boolean) {
    if (width <= 0 || height <= 0) { this.hasValidSize = false; return; }
    this.composer.setSize(width, height, _updateStyles);
    this.hasValidSize = true;
  }
