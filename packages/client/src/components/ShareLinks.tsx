/**
 * Shared list of social share platform buttons.
 * Used by both ShareButton (dropdown) and ShareModal (full modal).
 */
...
const PLATFORMS = [
  ...
  },
  {
    label: "EMAIL",
    getUrl: (text: string, url: string) =>
      `mailto:?subject=${encodeURIComponent("Check out this on Git Racer!")}&body=${encodeURIComponent(text + "\n\n" + url)}`,
    selfTarget: true as const,
    icon: (
      <svg className="w-5 h-5 text-arcade-gray shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function ShareLinks({ shareText, shareUrl, onShare, className }: ShareLinksProps) {
  const handleClick = (platform: (typeof PLATFORMS)[number]) => {
    if ("action" in platform && platform.action === "copy") {
    ...
    if ("getUrl" in platform && platform.getUrl) {
      ...
      if ("selfTarget" in platform && platform.selfTarget) {
  ...
  return (
    <div className={className}>
      {PLATFORMS.map((platform) => (
        ...
        >
          {platform.icon}
          <span className="font-pixel text-xs text-arcade-white">{platform.label}</span>
        </button>
      ))}
    </div>
  );
